...
def NN(matrices, labels, hidden_nodes, num_iters):
    ...
    X = tf.placeholder(shape=(None, 63), dtype=tf.float64, name='X')
    y = tf.placeholder(shape=(None, 1), dtype=tf.float64, name='y')

    W1 = tf.Variable(np.random.rand(63, hidden_nodes), dtype=tf.float64)
...
num_training = 1000
num_testing = 500
matrices, labels, percentAligned = parseData('/home/eadom/Grasp-Metrics/Grasp-Only-Data/')
